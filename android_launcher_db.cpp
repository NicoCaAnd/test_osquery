...
const std::string kPkgDb = "/data/data/com.sec.android.app.launcher/databases/launcher.db";

void genAndroidLaunchDb(sqlite3_stmt* stmt, Row& r) {
...
QueryData genAndroidLauncherDatabase(QueryContext& context) {
  QueryData results;
  ...
  std::string query = "SELECT _id, itemType, componentName, title, screen, rank, modified FROM appsTray;";
  sqlite3_stmt* stmt = nullptr;
  rc = sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr);
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    Row r;

    genAndroidLaunchDb(stmt, r);
