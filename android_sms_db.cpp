...
const std::string kPkgDb = "/data/data/com.android.providers.telephony/databases/mmssms.db";

void genAndroidSDb(sqlite3_stmt* stmt, Row& r) {
...
QueryData genAndroidSmsDatabase(QueryContext& context) {
  QueryData results;
  ...
  std::string query = "SELECT _id, transport_type, address, content, content_type, subject, date FROM messages;";
  sqlite3_stmt* stmt = nullptr;
  rc = sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr);
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    Row r;

    genAndroidSDb(stmt, r);
