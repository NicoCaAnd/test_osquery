...
const std::string kPkgDb = "/data/data/com.android.chrome/app_chrome/Default/Cookies";

void genAndroidChrCkDb(sqlite3_stmt* stmt, Row& r) {
...
QueryData genAndroidChromeCookiesDatabase(QueryContext& context) {
  QueryData results;
  ...
  std::string query = "SELECT creation_utc, host_key, value, expires_utc, is_secure, is_httponly, last_access_utc, source_port FROM cookies;";
  sqlite3_stmt* stmt = nullptr;
  rc = sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr);
  while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
    Row r;

    genAndroidChrCkDb(stmt, r);
